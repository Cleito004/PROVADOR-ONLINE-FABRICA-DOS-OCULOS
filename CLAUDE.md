...
`tests/astes.spec.mjs` is the exception to "DOM-level": it asserts the **temple geometry contract** (126 tris/side on square+aviator, 2 meshes/122 tris on cateye, a pivot on every temple, hinge fixed while shortening, stub at 0 yaw and full length at profile). It only works because it launches Chrome with `--use-fake-device-for-media-stream` — the GLBs are loaded inside `startApp()`, after `getUserMedia`, so with no camera there is nothing to measure. Use the same trick for any test that needs the render pipeline alive.
...
- **Temples are governed by two separate mechanisms** — *how much* temple is visible, and *which* temple is erased. They were one mechanism until v4.30.0, and that is why pitch was broken: the erasure is driven by the depth difference between the two sides, which is **zero in pure pitch**, so no threshold could ever have fixed it. Comparing app screenshots against photos of real glasses in the same poses showed the temples' *angle* was already correct (a temple pointing backwards really does project near-vertically when the head is pitched 40°) — what was missing is that a real head **hides** almost the whole temple in that pose, leaving a stub at the hinge, while the app drew it over the forehead and hair.
  ...
  - Shortening scales `mesh.scale.z` and compensates `mesh.position.z` by `hingeZ * (1 - k)` so the temple collapses **into the hinge** instead of toward its own middle. That requires each temple mesh to sit inside a **pivot `Group`** (`envolverEmPivo`) that carries the GLB node's transform, leaving the mesh's own transform free — the node transform varies per model, so scaling the mesh directly fought it.
  - `centroNaCena()` measures the **pivot's** `matrixWorld`, never the mesh's. Measuring the shrunken mesh would feed back (shrink → centroid moves → depth difference changes → shrink changes) and the temples would pulse on their own.
