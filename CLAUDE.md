...
- **Tracking smoothness (`window.SMOOTH`)** is an adaptive lerp: a low **floor** while the person is still (kills tremor) rising to a **ceiling** as they move. The two halves answer different complaints and must be tuned separately — the floor owns "it jitters when I stand still", the ceiling owns "it lags when I move fast". For a factor `a` per frame and a velocity `v`, the steady-state lag is `v * (1 - a) / a`, so **only the ceiling can fix lag**; lowering the floor just brings tremor back for nothing.
  - v4.34.0 fixed the reported "fast head movement arrives late": the ceiling was `0.65` (leaving `0.54 * v` of permanent lag, ~16 px at a brisk 30 px/frame) and is now `0.95` (`0.05 * v`). It is deliberately **not** `1.0` — fast motion blurs the image and MediaPipe's landmarks get noisy, and `1.0` would copy that noise straight onto the glasses.
  - The accelerator is measured in **face widths per frame**, not scene pixels. It used to be pixels (`posBoost: 0.02`), which meant it barely engaged at all — reaching the ceiling demanded ~21 px/frame — and, worse, made responsiveness **depend on distance**: someone further away moves fewer pixels and got less acceleration, exactly where the face is already smaller. Rotation was always dimensionless (`qDelta` returns radians) and only needed gain.
  - Scale has its **own** ceiling (`scaleMax`) and accelerator, kept low. It used to inherit the position's; a person's size does not change quickly, and letting it run as loose as position makes the glasses pulse on every measurement error.
  - The dead zone is a separate mechanism from the lerp and was left untouched — it governs the *still* case, and the fast-movement error is always far above it.
...
- Open issues are tracked in plain text at `PROBLEMAS PARA RESOLVER.txt`. Multi-person support is **done** (v4.23.0, `numFaces: MAX_FACES`, one glasses+occluder+smoothing set per face) — the note that once said it was open is obsolete. What remains open as of 30/07/2026 is camera/shop-window validation of v4.31–v4.34, not code.
